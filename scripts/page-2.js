...
const result = [];
const form = document.querySelector('.opros'); 
...
class Testbtn{
	...
	_getOptionsTemplate(i){
		const option = getTemplate('.option-template', '.option');
		option.text = i;
...
function testRedirect(){
	setTimeout(()=>{return window.location.href = 'page-3.html'}, 7000)
}
...
function moveLoading() {
	var elem = document.querySelector('.animation-sound_progress');
	var width = 0;
	const percent = document.querySelector('.animation-sound_percent');
...
function animationSound(){
	const screenSound = getTemplate('.sound-template', '.animation-sound');
